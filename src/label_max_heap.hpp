...
public:
    ...
    int get_out_degree(KeyType key) {
        IdxType cur = key2idx[key];
        return heap[cur].first;
    }
    ...
    bool get_max(ValueType& value, KeyType& key) {
        if (n > 0) {
            value = heap[n - 1].first;
            key = heap[n - 1].second;
            return true;
        } else
            return false;
